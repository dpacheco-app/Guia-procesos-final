    ...
    return (
        <form onSubmit={handleSubmit} className="relative">
            <input
                ...
                disabled={isLoading}
                aria-label="Buscar proceso constructivo"
            />
            <button
                type="submit"
                disabled={isLoading}
                ...
                aria-label={isLoading ? "Buscando, por favor espere" : "Buscar proceso constructivo"}
            >
                {isLoading ? (
                    'Buscando...'
                ) : (
                    <>
                        <SearchIcon className="w-5 h-5 mr-2" aria-hidden="true" />
                        Buscar
                    </>
                )}
            </button>
